...
class WebSocketKline():
    def __init__(self, symbol:str, candle:list, kline_type:Market):
        ...
        self.symbol =  symbol
        self.date_time = datetime.datetime.fromtimestamp(candle[0]) if kline_type == Market.SPOT else ""
...
class WebSocketDepthSpot():
    def __init__(self, symbol:str, asks:list, bids:list, ms_t:float):
        self.date_time = datetime.datetime.fromtimestamp(ms_t)
        self.symbol =  symbol
...
class WebSocketOrderProcess():
    def __init__(self, symbol, order_id, price, size, notional, side, order_type, ms_t, filled_size, filled_notional, margin_trading,
                        trade_order_type, state, last_fill_price, last_fill_count, last_fill_time, exec_type, detail_id, client_order_id):
        ...
        if int(trade_order_type) == 0:
            self.tradde_order_type = TradeOrderType.REGULAR
        elif int(trade_order_type) == 1:
            self.tradde_order_type = TradeOrderType.MAKER_ONLY
        elif int(trade_order_type) == 2:
            self.tradde_order_type = TradeOrderType.FILL_OR_KILL
        elif int(trade_order_type) == 3:
            self.tradde_order_type = TradeOrderType.IMMEDIATE_OR_CANCEL
        if int(state) == 4:
...
class WebSocketPositionFutures():
    def __init__(self, symbol, hold_volume, position_type, open_type, frozen_volume, close_volume, hold_avg_price,
        ...
        self.update_time = datetime.datetime.fromtimestamp(update_time)