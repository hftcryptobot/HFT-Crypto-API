...
class BitmartClient(PyClient):
    ...
    def get_futures_position_details(self, symbol):
        response =  self._request_with_params(GET, FUTURES_CURRENT_POSITION, {'symbol': symbol})
        order_positions = []
        for position in  json.loads(response.content)['data']:
            order_pos = OrderPosition(symbol)
            order_pos.leverage = float(position["leverage"])
            order_pos.date_time = datetime.datetime.fromtimestamp(int(position["timestamp"]))
            order_pos.current_fee = float(position["current_fee"])
            order_pos.open_date_time = datetime.datetime.fromtimestamp(int(position["open_timestamp"]))
    ...
    def cancel_order(self, symbol: str, order_id="", client_order_id="", market=Market.SPOT):
        param = {
            'symbol': symbol,
            'order_id': order_id,
    ...
    def update_order_detail(self, order:BitmartOrder, market=Market.SPOT):
        ...
        if market == Market.FUTURES:
            ...
            order.update_time = datetime.datetime.fromtimestamp(int(json.loads(response.content)['data']['update_time']))
            ...
            order.create_time = json.loads(response.content)['data']['create_time']
            order.update_time = datetime.datetime.fromtimestamp(datetime.now())

        return True

    # GET https://api-cloud.bitmart.com/spot/v3/orders
    def get_order_history(self, symbol: str, status: OrderState, market=Market.SPOT, N=200, start_time="", end_time=datetime.now):
        ...
        if market == Market.SPOT:
            ...
            for order in json.loads(response.content)['data']['orders']:
                ...
                bt_order.create_time = datetime.datetime.fromtimestamp(float(order["create_time"]))
            ...
            for order in json.loads(response.content)['data']:
                ...
                size = order["size"]
                price = order["price"]
                bt_order = BitmartOrder(symbol, side, size, price, market=Market.FUTURES)
                bt_order.order_id = order["order_id"]
                bt_order.order_status = OrderState.STATUS_CHECK if int(order["state"]) == 2 else OrderState.ORDER_SUCCESS
                bt_order.order_type = OrderType.LIMIT if order["type"] == "limit" else OrderType.MARKET
                bt_order.leverage = int(order["leverage"])
                bt_order.open_type = OrderOpenType.CROSS if order["open_type"] == "cross" else OrderOpenType.ISOLATED
                bt_order.price_avg = float(order["deal_avg_price"])
                bt_order.create_timeorder["create_time"]
                order_objects.append(bt_order)
        
        return order_objects

    # Unified methods for trading
    def submit_order(self, symbol, side, price:str, size:int, client_order_id="", order_type=OrderType.LIMIT, market=Market.SPOT, leverage=1, open_type=OrderOpenType.ISOLATED):
        
        bm_order = BitmartOrder(market, symbol, side, size, price, order_type, leverage, open_type, client_order_id)
        if market == Market.SPOT or market == Market.SPOT_MARGIN:
            ...
            try:
                ...
                bm_order.order_id = "Code " + json.loads(response.content)['code'] + ": " + json.loads(response.content)['message']
                print("Wrong request")
                return bm_order
            return bm_order
        elif market == Market.FUTURES:
    ...
    def get_contract(self, symbol:str, market=Market.FUTURES):
        ...
        return BitmartFutureContract(symbol, product_type, base_currency, quote_currency, volume_precision, price_precision, max_volume, min_volume,
