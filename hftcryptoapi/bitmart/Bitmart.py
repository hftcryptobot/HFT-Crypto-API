...
class BitmartClient(PyClient):
    ...
    def __init__(
        ...
        self.ws_private: Dict[Union[Market], BitmartWs] = {
            ...
            Market.FUTURES: BitmartWs(
                CONTRACT_WS_URL_USER, Market.FUTURES, api_key, memo, secret_key
            ),
        }

    def subscribe_private(
        self, market: Market, channels: List[str], symbols: Optional[List[str]] = None
    ):
    ...
    def get_service_status(self) -> List[BitmartService]:
        ...
        for bt_service in json.loads(response.content)["data"]["service"]:
            ...
            services.append(
                BitmartService(title, service_type, status, start_time, end_time)
            )
        return services
    ...
    def get_currency_list(self):
        ...
        for currency in json.loads(response.content)["data"]["currencies"]:
            ...
            bitmart_currencies.append(
                Currency(id, name, withdraw_enabled, deposit_enabled)
            )
    ...
    def get_spot_trade_fee_rate(self, symbol) -> BitmartTradeFee:
        response = self._request_with_params(
            GET, ACCOUNT_TRADE_FEE, {"symbol": symbol}, Auth.KEYED
        )
    ...
    def get_spot_ticker_details(self, symbol: str) -> SpotTickerDetails:
        response = self._request_with_params(
            GET, SPOT_TICKER_DETAILS, {"symbol": symbol}
        )
        data = json.loads(response.content)["data"]
    ...
    def get_futures_position_details(self, symbol) -> List[OrderPosition]:
        response = self._request_with_params(
            GET, FUTURES_CURRENT_POSITION, {"symbol": symbol}, Auth.SIGNED
        )
        order_positions = []
    ...
    def cancel_all_orders(
        self, symbol: str, side: SpotSide, market=Union[Market.SPOT, Market.FUTURES]
    ) -> bool:
        param = {"symbol": symbol, "side": side.value}
        url = (
            SPOT_CANCEL_ALL_ORDERS
            if market == Market.SPOT
            else FUTURES_CANCEL_ALL_ORDERS
        )
    ...
    def update_order_details(self, order: BitmartOrder) -> BitmartOrder:
        ...
        if market == Market.FUTURES:
            ...
            response = self._request_with_params(
                GET, FUTURES_ORDER_DETAIL, param, Auth.KEYED
            )
            data = json.loads(response.content)["data"]
            ...
            response = self._request_with_params(
                GET, SPOT_GET_ORDER_DETAILS, param, Auth.KEYED
            )
            data = json.loads(response.content)["data"]
            order.price = float(data["price"])
    ...
    def get_order_details(
        self, symbol: str, order_id: str, market=Market.SPOT
    ) -> BitmartOrder:
    ...
    def get_order_history(
        ...
        if market == Market.SPOT:
            ...
            response = self._request_with_params(
                GET, SPOT_USER_ORDER_HISTORY, param, Auth.KEYED
            )
            for order in json.loads(response.content)["data"]["orders"]:
                ...
                bt_order.create_time = datetime.fromtimestamp(
            ...
            response = self._request_with_params(
                GET, FUTURES_ORDER_HISTORY, param, Auth.KEYED
            )
            for order in json.loads(response.content)["data"]:
                ...
                size = order["size"]
                price = order["price"]
                bt_order = BitmartOrder(
                    symbol, FuturesSide(side), size, price, market=Market.FUTURES
                )
                bt_order.order_id = order["order_id"]
    ...
    def submit_order(
        self,
        symbol: str,
        side: Union[FuturesSide, SpotSide],
        ...
        if market == Market.SPOT:
            response = self._request_with_params(
                POST, SPOT_PLACE_ORDER, bm_order.param, Auth.SIGNED
            )
        elif market == Market.SPOT_MARGIN:
            response = self._request_with_params(
                POST, SPOT_MARGIN_PLACE_ORDER, bm_order.param, Auth.SIGNED
            )
        else:  # market == Market.FUTURES:
            response = self._request_with_params(
                POST, FUTURES_SUBMIT_ORDER, bm_order.param, Auth.SIGNED
            )
    ...
    def get_futures_open_interest(self, symbol: str) -> FuturesOpenInterest:
        response = self._request_with_params(
            GET, FUTURES_OPEN_INTEREST, {"symbol": symbol}
        )
        data = json.loads(response.content)["data"]
    ...
    def get_futures_funding_rate(self, symbol: str) -> FuturesFundingRate:
        response = self._request_with_params(
            GET, FUTURES_FUNDING_RATE, {"symbol": symbol}
        )
        data = json.loads(response.content)["data"]
    ...
    def get_account_balance(self, market=Market.SPOT) -> BitmartWallet:
        ...
        if market == Market.SPOT:
            ...
            for currency in json.loads(response.content)["data"]["wallet"]:
                ...
                wallet_currencies.append(
                    WalletItem(symbol, available, frozen, wallet_type=Market.SPOT)
                )
        elif market == Market.FUTURES:
            response = self._request_without_params(
                GET, FUTURES_CONTRACT_ASSETS_DETAIL, Auth.KEYED
            )
            ...
            response = self._request_without_params(
                GET, ACCOUNT_MARGIN_DETAILS, Auth.KEYED
            )
            for ticker in json.loads(response.content)["data"]["symbols"]:
                ...
                margin_acc_symbol.liquidate_price = float(
                    ticker.get("liquidate_price", 0) or 0
                )
    ...
    def close_futures_position(
        ...
        open_positions = [
            p for p in positions if p.symbol == symbol and p.current_amount != 0
        ]

        result = False
        for p in open_positions:
            if (not is_cross and p.position_cross) or (
                is_cross and not p.position_cross
            ):
                continue
            ...
            order_open_type = (
                OrderOpenType.CROSS
                if is_cross and p.position_cross
                else OrderOpenType.ISOLATED
            )
    ...
    def spot_margin_get_borrow_record(
        ...
        response = self._request_with_params(
            GET, MARGIN_BORROW_RECORD, params=params, auth=Auth.KEYED
        )
        return [
            BorrowRecord(**r) for r in json.loads(response.content)["data"]["records"]
        ]

    def spot_margin_get_repay_record(
        self,
        symbol: str,
        ...
        response = self._request_with_params(
            GET, MARING_REPAYMENT_RECORD, params=params, auth=Auth.KEYED
        )
        return [
            RepayRecord(**r) for r in json.loads(response.content)["data"]["records"]
        ]

    def spot_margin_borrowing_rate(self, symbol: str) -> List[BorrowingRateAndAmount]:
        response = self._request_with_params(
            GET,
            MARGIN_TRADING_PAIR_BORROW_RATE_AND_AMOUNT,
            params=dict(symbol=symbol),
            auth=Auth.KEYED,
        )
        return [
            BorrowingRateAndAmount(**r)
            for r in json.loads(response.content)["data"]["symbols"]
        ]
