...
if __name__ == "__main__":
    ...
    depth_futures = client.get_symbol_depth(
        symbol=symbol_spot, precision=6, size=50, market=Market.SPOT
    )
    depth_spot = client.get_symbol_depth(
        symbol=symbol, precision=6, size=50, market=Market.FUTURES
    )
    ...
    client.subscribe_private(
        Market.FUTURES, [BtFuturesTPrivateAssetChannel], ["ETH", "USDT"]
    )
    client.subscribe_public(Market.FUTURES, [BtFuturesTickerChannel])
    client.subscribe_public(
        Market.FUTURES,
        [
            BtFuturesSocketKlineChannels.K_LINE_CHANNEL_1HOUR,
            BtFuturesSocketDepthChannels.DEPTH_CHANNEL_5LEVEL,
        ],
        [symbol],
    )

    client.start_websockets(
        Market.FUTURES, on_message=lambda message: print(f"Message: {message}")
    )
    client.subscribe_public(
        Market.SPOT,
        [
            BtSpotSocketKlineChannels.K_LINE_CHANNEL_1HOUR,
    ...
    client.start_websockets(
        Market.SPOT, on_message=lambda message: print(f" {message}")
    )
    ...
    amount = [p for p in positions if p.symbol == "ETHUSDT" and p.current_amount != 0][
        0
    ].current_amount
